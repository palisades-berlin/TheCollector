...
- All v2.1 and v3.0 features inherit the Design System 2.0 token layer without a migration
  cost.
- ~~Figma remains the UX/UI single source of truth~~ — superseded by ADR 0015 during a temporary Penpot period, then restored by ADR 0016. Figma is now the UX/UI single source of truth again (see `docs/project-ruleset.md`).
