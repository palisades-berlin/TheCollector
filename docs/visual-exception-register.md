...
| `urls-library-desktop.png`          | `400`             | URL Library desktop header/filter rendering shows minor cross-runner drift on GitHub macOS parity node (recent CI diff: `212`).                  | `<= 100`          |
| `urls-library-narrow.png`           | `400`             | URL Library narrow filter/header rendering shows minor cross-runner drift on GitHub macOS parity node (recent CI diff: `206`).                   | `<= 100`          |
| `history-default.png`               | `6400`            | History header/filter row still has platform font/rendering variance (latest GitHub macOS CI diff: `6320`).                                      | `<= 100`          |
| `history-empty.png`                 | `5800`            | Empty-state card typography and panel spacing drift significantly on hosted macOS parity node.                                                   | `<= 100`          |
| `history-loading.png`               | `4800`            | Skeleton/loading gradients and text antialiasing vary strongly across render backends (latest CI diff observed: `4720`).                         | `<= 100`          |
