...
test('maps roadmap features to expected tiers', () => {
  ...
  assert.equal(getRequiredTier('url_notes'), CAPABILITY_TIER.PRO);
...
test('enforces hierarchical gating by capabilityTier', () => {
  ...
  assert.equal(canUseFeature('url_notes', basic), false);
  ...
  assert.equal(canUseFeature('url_notes', pro), true);
  ...
  assert.equal(canUseFeature('url_notes', ultra), true);
...
test('returns deterministic capability snapshot and feature lists', () => {
  ...
  assert.ok(features.pro.includes('url_notes'));
